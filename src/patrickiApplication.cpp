...
#include <stdlib.h>
#include <time.h>
#include <assignment3/patricki.h> 
#include <assignment3/lynxmotionUtilities.h>


int main(int argc, char ** argv) {
  
   ros::init(argc, argv, "patricki"); // Initialize the ROS system
...
while(counter < 4){
   ...
   objects[counter] = ob;
   if (end_of_file == EOF) {   
	  printf("Fatal error: One unable to read the object position and orientation\n");
      prompt_and_exit(1);
   }
   printf("Object pose is reading yeahhhhh %d", counter);
   ...
   if (end_of_file == EOF) {   
	  printf("Fatal error: Two unable to read the destination position and orientation\n");
      prompt_and_exit(1);
   }
...
for(int i =0; i<3 ; i++){
   ...
   object          = trans(objects[i].x, objects[i].y, objects[i].z)      * rotz(objects[i].phi);      // object pose
  
   // object          = trans(object_x,      object_y,      object_z)      * rotz(object_phi);      // object pose
   
   float theY = destination_y;
   float theX = destination_x;
   
   if(i == 0){
      theY = destination_y - 3;
      theX = destination_x + 2.3;
   }
   if( i == 2){
      theX = destination_x - 1.5;
   }
   destination     = trans(theX, theY, -18+(11.4*i)) * rotz(destination_phi); // destination pose
