...
main(int argc, char **argv) {
  ...
  /* construct the full path and filename */
   ...
   //  devidConquerLocomotion(current_x, current_y, current_theta, goal_x, goal_y, pub);
         float xr = current_x;
         float yr = current_y;
         float dx = goal_x - xr;
         float dy = goal_y - yr;
         ...
         // devidConquer(xr, yr, start_theta, goal_x,  goal_y,  goal_theta);
         ...
         do{
               ...
               // printf(" %f ", teleport_arguments.request.theta);
               if(abs(head_err) >0.10){
                  msg.linear.x = 0;
   ...
   //  printf("The current pos : %f, %f, %f", teleport_arguments.request.x, teleport_arguments.request.y, teleport_arguments.request.theta);
	...
	/* MIMO algorithm */
         ...
         do{
               ...
               // if(abs(head_err) >0.10){
                  // msg.linear.x = 0;
                  msg.angular.z = - abs(head_err * 1.7);
               // }else{
                  msg.linear.x = abs(pos_err * 0.7);
                  // msg.angular.z = 0;
               // }
               pub.publish(msg);
               ros::spinOnce();
