...
#include <assignment2/patricki.h> 
...
void devidConquerLocomotion(float in_rx, float in_ry, float goal_x, float goal_y, const ros::Publisher pub){
         ...
         do{
               ...
               if(abs(head_err) >0.10){
                  msg.linear.x = 0;
                  msg.angular.z = - abs(head_err * 0.6);
               }else{
...
void mimoLocomotion(float in_rx, float in_ry, float goal_x, float goal_y, const ros::Publisher pub){
         float xr = in_rx;
         float yr = in_ry;
         float dx = goal_x - xr;
         float dy = goal_y - yr;
